...
import {currencyListAtom, selectedCurrencyAtom} from "../../../../lib/Atom/walletProcess/walletProcess.ts";
import "./style.css"
...
export const Currency: React.FC = () => {
    ...
    const [currencyList, _setCurrencyList] = useAtom(currencyListAtom)
    ...
    return <div className="flex w-full mt-6 flex-col gap-2">
        <span className="text-[14px] font-medium px-2">
        ...
        <div
            ...
            <div
                ...
                className={`px-4 flex currency-container-dont-close justify-between items-center bg-[#F8F9FB]  w-full rounded-[48px] h-[50px] ${currencyList.length > 0 ? '' : 'pointer-events-none'} cursor-pointer`}>
                {currencyList.length > 0 ? (<>
                    <span
                        className={"text-[14px] currency-container-dont-close text-wallet-disable-text"}>{currencyList[selectedCurrency || 0]}</span>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                         className="currency-container-dont-close text-wallet-disable-text" viewBox="0 0 16 16">
