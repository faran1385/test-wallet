...
export const ImportPhrases: React.FC<ImportPhrasesProps> = (T) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [_mnemonic, setMnemonic] = useAtom(userMnemonic);
  ...
  const pasteHandler = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    const rawPastedTextArray = pastedText
      .split(" ")
    ...
    if (rawPastedTextArray.length === T.selectedPhraseCount) {
      ...
      const pastedArray = rawPastedTextArray.map((text) => {
        if (
          Number.isInteger(+text.trim().slice(0, 1)) &&
          text.slice(1, 2) === "-"
        ) {
          return text.slice(2);
        } else if (
          Number.isInteger(+text.trim().slice(0, 2)) &&
          text.slice(2, 3) === "-"
        ) {
          return text.slice(3);
        }
        ...
        <div className={"sm:mt-8 my-auto"}>
          <div className="flex flex-col items-center justify-center gap-3">
            <h2 className="text-2xl font-bold text-center leading-[30px]">
              Enter your Secret Recovery
              <br className="pt-2" />
              Phrase
            </h2>
            <p className="text-[14px]  font-normal text-[#686D74] text-center">
              Use the fields below to enter your Secret Recovery Phrase.
              <br /> Pay attention to the words and their order and ensure they
              are correct when entering.
            </p>
          </div>
          <div className="sm:block grid mt-8 gap-4">
            <div className="grid  px-4 import-phrases-containe overflow-y-auto palce-items-center gap-4 grid-cols-3 sm:gap-5 text-[12px] sm:text-[14px]">
