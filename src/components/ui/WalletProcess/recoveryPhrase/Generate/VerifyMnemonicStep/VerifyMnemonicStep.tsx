...
export const VerifyMnemonicStep: React.FC<VerifyMnemonicStepProps> = (T) => {
  ...
  const [_mnemonic, _setMnemonic] = useAtom(userMnemonic);
  ...
  const [randomIndexArray, setRandomIndexArray] = useState(
    getRandomNumberArray(3, T.selectedPhraseCount)
  );

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [_recoveryProcess, setRecoveryProcess] = useAtom(
    recoveryGenerateProcessAtom
  );
  ...
  const chooseHandler = (selectedPhrase: string) => {
    const phrase =
      phrases[`${selectedPhraseCount}`][randomIndexArray[verifyIndex]];
  ...
  const [phrasesShuffledArray, setPhrasesShuffledArray] = useState(
    shuffleArray(phrases[`${selectedPhraseCount}`])
  );
      ...
      <div className="w-full h-full sm:pb-0 pb-8 flex flex-col items-center">
        <div className={"sm:mt-8 my-auto"}>
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <div
              className={
                "grid my-2 px-4 phrases-container overflow-y-auto palce-items-center gap-4 grid-cols-3 sm:gap-5 text-[12px] sm:text-[14px]"
              }
            >
            ...
            <p className="text-[14px]  font-normal text-[#686D74] text-center">
              Let’s make sure you remember your phrase correctly.
              <br />
              Select your ({getThNumber(randomIndexArray[0] + 1)}) , (
              {getThNumber(randomIndexArray[1] + 1)}) & (
              {getThNumber(randomIndexArray[2] + 1)}) words of your secret
              recovery phrase.
            </p>
          </div>
          <div className="sm:block grid mt-8 gap-4">
            <div className="grid  px-4 pt-1 phrases-container overflow-y-auto palce-items-center gap-4 grid-cols-3 sm:gap-5 text-[12px] sm:text-[14px]">
        ...
        <div className="w-full flex flex-col items-center sm:mt-12 gap-7">
          <div className="w-full flex flex-col-reverse sm:flex-row items-center justify-between gap-12">
            ...
            <button
              disabled={!isValid}
              onClick={() => {
                console.log(phrasesShuffledArray.join(" "));
                _setMnemonic(phrases?.[12]?.join(" "));
              ...
              className={`text-nowrap text-center w-full duration-300 ${
                isValid
                  ? "bg-[#24D998] hover:bg-[#21C58A]"
                  : "bg-wallet-disable-background text-wallet-disable-text opacity-60 cursor-not-allowed"
              }  rounded-[40px] py-3 text-base font-normal peer-checked:hidden`}
            >
              Continue
            </button>
          </div>
