...
interface SelectMnemonicLengthStepProps {
  ...
  setRecoveryProcess:
    | React.Dispatch<React.SetStateAction<recoveryGenerateProcessType>>
    | React.Dispatch<React.SetStateAction<recoveryImportProcessType>>;
  selectedPhraseCount: 12 | 15 | 24;
}

export const SelectMnemonicLengthStep: React.FC<
  SelectMnemonicLengthStepProps
> = (T) => {
    ...
    <>
      <div className="w-full h-full sm:pb-0 pb-8 flex flex-col items-center">
        <div className={"sm:mt-8 my-auto"}>
          ...
          <div className="grid palce-items-center gap-4 grid-cols-3 sm:gap-5 pt-8 text-[12px] sm:text-[14px]">
            {[12, 15, 24].map((length, i) => {
              return (
                <RecoveryPhraseButton
                  setSelectedPhraseCount={T.setSelectedPhraseCount}
                  text={`${length}`}
                  key={i}
                  checked={{
        ...
        <div className="w-full flex flex-col items-center mt-0 sm:mt-12 gap-7">
          <div className="w-full flex flex-col-reverse sm:flex-row items-center justify-between gap-12">
            ...
            <button
              onClick={() =>
                T.setRecoveryProcess(
                  processType === "import"
                    ? ("importPhrases" as any)
                    : ("displayMnemonicWords" as any)
                )
              }
