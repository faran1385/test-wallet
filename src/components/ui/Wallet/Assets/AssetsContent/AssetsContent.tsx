...
export const AssetsContent = ({ assets, address, networkType }: any) => {
  // const [assets] = useAtom(assetsAtom)

  return (
    <div className="mt-4 sm:mt-7 flex flex-col gap-2">
      <div
        className={
          "wallet-asset-container px-1 gap-2 flex flex-col overflow-y-auto"
        }
      >
        {assets.length > 0
          ? assets.map((asset: any, i: any) => {
              return <Asset key={i} usage={"main"} req={{ ...asset }} address={address} networkType={networkType} />;
            })
          : [1, 2, 3, 4, 5].map((value) => {
              return (
                <div
      ...
      <div className="mb-[90px] md:hidden relative flex flex-col items-center mt-4 sm:hiddenz">
        <div className="w-full h-[72px] px-4 py-2 bg-white rounded-lg shadow border-b border-[#ebeef1] inline-flex justify-start items-center gap-3 z-[3]">
          <div className="grow shrink basis-0 h-10 justify-start items-center gap-3 flex">
            ...
            <div className="grow shrink basis-0 text-black text-sm font-medium font-['Plus Jakarta Sans'] leading-[18px] tracking-tight">
              NFTs, Stocks, Commodities
            </div>
          </div>
          <div className="w-[45px] text-sm font-medium font-['Plus Jakarta Sans'] leading-[18px] tracking-tight">
            $0.00
          </div>
        </div>
        <div className="w-[94%] h-[72px] bg-white rounded-lg shadow border-b border-[#ebeef1] absolute top-2 z-[2]"></div>
        <div className="w-[88%] h-[72px] bg-white rounded-lg shadow border-b border-[#ebeef1] absolute top-4"></div>
      </div>
    </div>
  );
};
